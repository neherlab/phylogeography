...
if __name__=="__main__":
    ...
    for line_style, linear_bins in zip(['-', '--', '-.'], [3,5,10]):
        ...
        plt.plot(D_array, res_mean, label=f'free diffusion, dx=L/{linear_bins}', c='C0', ls=line_style)
        plt.plot(D_array, np.ones_like(D_array)*np.sqrt(nbins/ntips), label=f'well mixed limit dx=L/{linear_bins}', c='C1', ls=line_style)
