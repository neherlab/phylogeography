...
Diffusion constants estimated from the population using Eq.~\ref{eq:dispersal} remain accurate with moderate overestimation of small diffusion constants.
...
To explore such the result of such changing habitats on phylogeographic inferences, we simulated a habitat with a Gaussian carrying capacity that moves at a constant velocity $v$ along the $x$-axis (Fig.~\ref{fig:traveling_wave}) with periodic boundary conditions.
When varying the microscopic dispersal parameter $D$ and the velocity $v$ of the carrying capacity profile, I observe diverse behavior with often dramatically overestimated $\hat{D}$. These behaviors are explained comparing the speed $v_{FKPP}$ at which the population can invade new territory with the external speed $v$, the ratio of which is used to plot the simulation data in Fig.~\ref{fig:traveling_wave}.
